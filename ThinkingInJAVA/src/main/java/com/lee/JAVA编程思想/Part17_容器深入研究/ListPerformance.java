...
public class ListPerformance {
	...
	static {
		...
		tests.add(new Test<List<Integer>>("remove") {
			...
			int test(List<Integer> list, TestParam tp) {
				...
				for (int i = 0; i < loops; i++) {
					list.clear();
					//	list.addAll(new CountingIntergerList(size));
					while (list.size() > 5) {
						list.remove(5);
					}
				}
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			Tester.defultParams = TestParam.array(args);
		}
...
/*			@Override
		...
		arrayTest.timedTest();
		;
		Tester.defultParams = TestParam.array(10, 5000, 100, 5000, 1000, 1000, 10000, 200);
		if (args.length > 0) {
			Tester.defultParams = TestParam.array(args);
		}
